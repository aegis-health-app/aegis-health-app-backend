...
export class SettingService {
  ...
  async changeUserPassword(passwordInterface: ChangePassword, uid: number): Promise<boolean> {
    ...
    if (!isMatched) throw new HttpException('Old password entered is incorrect', HttpStatus.CONFLICT);
  ...
  async changePhoneNumber(phoneInterface: ChangePhoneNo, uid: number, token: string): Promise<boolean> {
    ...
    if (oldPhoneNumber === newPhoneNumber) throw new HttpException('Old phone number is the new phone number', HttpStatus.BAD_REQUEST);
    const otpVerified = this.otpService.verifyOtp(token, phoneInterface.enteredPin);
    if (!otpVerified) throw new HttpException('PIN entered is incorrect', HttpStatus.FORBIDDEN);
